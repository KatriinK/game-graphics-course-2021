...
import {positions, normals, indices} from "../blender/plane.js"
...
let skyboxArray = app.createVertexArray()
    .vertexAttributeBuffer(0, app.createVertexBuffer(PicoGL.FLOAT, 5, skyboxPositions))
    .indexBuffer(app.createIndexBuffer(PicoGL.UNSIGNED_INT, 5, skyboxTriangles));

let mirrorArray = app.createVertexArray()
    .vertexAttributeBuffer(0, app.createVertexBuffer(PicoGL.FLOAT, 3, mirrorPositions))
    .vertexAttributeBuffer(1, app.createVertexBuffer(PicoGL.FLOAT, 3, mirrorUvs))
...
(async () => {
    const cubemap = app.createCubemap({
        negX: await loadTexture("galaxy.jpg"),
        posX: await loadTexture("galaxy.jpg"),
        negY: await loadTexture("galaxy.jpg"),
        posY: await loadTexture("galaxy.jpg"),
        negZ: await loadTexture("galaxy.jpg"),
        posZ: await loadTexture("galaxy.jpg")
    });
    ...
    function draw() {
        ...
        vec3.rotateY(cameraPosition, vec3.fromValues(0, 4, 2.5), vec3.fromValues(0, 0, 0), time * 0.05);
        ...
        mat4.fromXRotation(rotateXMatrix, -2.2);
        mat4.fromYRotation(rotateYMatrix, time * .2354);
